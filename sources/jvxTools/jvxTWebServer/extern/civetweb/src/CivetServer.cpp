...
bool CivetHandler::handleWebSocketConnect(CivetServer *server, const struct mg_connection *conn)
{
	UNUSED_PARAMETER(server);
	UNUSED_PARAMETER(conn);
	return false;
...
void CivetHandler::handleWebSocketReady(CivetServer *server, struct mg_connection *conn)
{
	UNUSED_PARAMETER(server);
	UNUSED_PARAMETER(conn);
}

bool CivetHandler::handleWebSocketData(CivetServer *server, struct mg_connection *conn, int, char *, size_t)
{
	UNUSED_PARAMETER(server);
	UNUSED_PARAMETER(conn);
	return false;
...
void CivetHandler::handleWebSocketClose(CivetServer *server, const struct mg_connection *conn)
{
	UNUSED_PARAMETER(server);
	UNUSED_PARAMETER(conn);
}
...
int CivetServer::websocket_connect_handler(const struct mg_connection * conn, void *cbdata)
{
	const struct mg_request_info *request_info = mg_get_request_info(conn);
	assert(request_info != NULL);
	CivetServer *me = (CivetServer *)(request_info->user_data);
...
*/
	...
	if (handler) 
	{
		// If returns bool true, connection will resist
		// STRANGE: civetweb expects returned 0 to follow connect..
		return handler->handleWebSocketConnect(me, conn) ? 0 : 1;
	}

	return 0; // No handler found
}

void CivetServer::websocket_ready_handler(struct mg_connection * conn, void *cbdata)
{
	const struct mg_request_info *request_info = mg_get_request_info(conn);
	assert(request_info != NULL);
	CivetServer *me = (CivetServer *)(request_info->user_data);
...
*/
	...
	if (handler) 
	{
		handler->handleWebSocketReady(me, conn);
	}

}

int CivetServer::websocket_data_handler(struct mg_connection *conn, int purpose, char *payload, size_t l_payload, void *cbdata)
{
	const struct mg_request_info *request_info = mg_get_request_info(conn);
	assert(request_info != NULL);
	CivetServer *me = (CivetServer *)(request_info->user_data);
...
*/
	...
	{
		// If returns bool true, connection will resist
		return handler->handleWebSocketData(me, conn, purpose, payload, l_payload) ? 1 : 0;
	}

	return 0; // No handler found
}

void CivetServer::websocket_close_handler(const struct mg_connection *conn, void *cbdata)
{
	const struct mg_request_info *request_info = mg_get_request_info(conn);
	assert(request_info != NULL);
	CivetServer *me = (CivetServer *)(request_info->user_data);
...
*/
	...
	if (handler) 
	{
		handler->handleWebSocketClose(me, conn);
	}
}
...
void CivetServer::addRequestHandler(const std::string &uri, CivetHandler *handler)
{
	mg_set_request_handler(context, uri.c_str(), requestHandler, handler);
}

void CivetServer::addWebsocketHandler(const std::string &uri, CivetHandler *handler)
{
...
void CivetServer::removeRequestHandler(const std::string &uri)
{
...
void 
CivetServer::triggerStop()
{
	mg_trigger_stop(context);
}

bool 
CivetServer::waitStopComplete()
{
	bool res = mg_wait_stop(context);
	if(res == true)
	{
		context = NULL;
	}
