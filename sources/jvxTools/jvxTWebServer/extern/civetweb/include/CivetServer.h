...
class CIVETWEB_API CivetHandler
{
	...
	virtual bool handleWebSocketConnect(CivetServer *server, const struct mg_connection *conn);
	virtual void handleWebSocketReady(CivetServer *server, struct mg_connection *conn);
	virtual bool handleWebSocketData(CivetServer *server, struct mg_connection *conn, int, char *, size_t);
	virtual void handleWebSocketClose(CivetServer *server, const struct mg_connection *conn);

	//virtual bool handleWebsocket(CivetServer *server, struct mg_connection *conn);
...
class CIVETWEB_API CivetServer
{
	 ...
	 */
	void addRequestHandler(const std::string &uri, CivetHandler *handler);

	void addWebsocketHandler(const std::string &uri, CivetHandler *handler);
	...
	*/
	...
	static int websocket_connect_handler(const struct mg_connection *, void *);
	static void websocket_ready_handler(struct mg_connection *, void *);
	static int websocket_data_handler(struct mg_connection *, int, char *, size_t, void *);
	static void websocket_close_handler(const struct mg_connection *, void *);
