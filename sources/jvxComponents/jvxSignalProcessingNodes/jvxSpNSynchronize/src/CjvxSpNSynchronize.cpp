...
CjvxSpNSynchronize::CjvxSpNSynchronize(JVX_CONSTRUCTOR_ARGUMENTS_MACRO_DECLARE) :
...
CjvxSpNSynchronize::~CjvxSpNSynchronize()
{
}

// ============================================================================
jvxErrorType 
CjvxSpNSynchronize::select(IjvxObject* owner)
{
	jvxErrorType res = CjvxBareNode1ioRearrange::select(owner);
	if (res == JVX_NO_ERROR)
	{
		genSynchronize::init__config_general();
		genSynchronize::allocate__config_general();
		genSynchronize::register__config_general(this);
	}
	return res;
}

jvxErrorType 
CjvxSpNSynchronize::unselect()
{
	jvxErrorType res = CjvxBareNode1ioRearrange::unselect();
	if (res == JVX_NO_ERROR)
	{
		genSynchronize::unregister__config_general(this);
		genSynchronize::deallocate__config_general();
	}
	return res;
}

jvxErrorType
CjvxSpNSynchronize::activate()
{
	...
	if (res == JVX_NO_ERROR)
	{
		...
		oneInputConnectorElement elmIn;
		elmIn.refCnt = 0;
		elmIn.theConnector = &sec_master;		
		_common_set_conn_factory.input_connectors[&sec_master] = elmIn;

		oneOutputConnectorElement elmOut;
		elmOut.refCnt = 0;
		elmOut.theConnector = &sec_master;
		_common_set_conn_factory.output_connectors[&sec_master] = elmOut;
		
		bufferMode = genSynchronize::translate__config_general__mode_from();
		genSynchronize::unregister__config_general(this);
		genSynchronize::deallocate__config_general();

		genSynchronize::init_all();
		genSynchronize::allocate_all();
		genSynchronize::register_all(this);
...
jvxErrorType
CjvxSpNSynchronize::deactivate()
{
	jvxErrorType res = CjvxBareNode1ioRearrange::_pre_check_deactivate();
	if (res == JVX_NO_ERROR)
	{
		CjvxProperties::_undo_update_property_access_type(genSynchronize::config_general.mode);

		genSynchronize::unregister_all(this);
		genSynchronize::deallocate_all();	

		genSynchronize::allocate__config_general();
		genSynchronize::register__config_general(this);
		genSynchronize::translate__config_general__mode_to(bufferMode);
		
		_common_set_conn_factory.input_connectors.erase(&sec_master);
		_common_set_conn_factory.output_connectors.erase(&sec_master);

		sec_master.deactivate();
		res = CjvxBareNode1ioRearrange::deactivate();
	}
	return res;
}

jvxErrorType CjvxSpNSynchronize::test_connect_icon(JVX_CONNECTION_FEEDBACK_TYPE(fdb))
{
...
jvxErrorType 
CjvxSpNSynchronize::start_connect_icon(JVX_CONNECTION_FEEDBACK_TYPE(fdb))
{
	auto res = CjvxBareNode1ioRearrange::start_connect_icon(JVX_CONNECTION_FEEDBACK_CALL(fdb));
	if (res == JVX_NO_ERROR)
	{
		sec_master.try_allocate_runtime_sync_data(0);
	}
	return res;
}

jvxErrorType 
CjvxSpNSynchronize::stop_connect_icon(JVX_CONNECTION_FEEDBACK_TYPE(fdb))
{
	auto res = CjvxBareNode1ioRearrange::stop_connect_icon(JVX_CONNECTION_FEEDBACK_CALL(fdb));
	if (res == JVX_NO_ERROR)
	{
		sec_master.try_deallocate_runtime_sync_data(0);
	}
	return res;
}

jvxErrorType 
CjvxSpNSynchronize::process_buffers_icon(jvxSize mt_mask, jvxSize idx_stage)
{
...
jvxErrorType 
CjvxSpNSynchronize::request_hidden_interface(jvxInterfaceType tp, jvxHandle** hdl)
{
	jvxErrorType res = JVX_NO_ERROR;
	...
	case JVX_INTERFACE_CONNECTOR_MASTER_FACTORY:
		*hdl = reinterpret_cast<jvxHandle*>(static_cast<IjvxConnectionMasterFactory*>(&sec_master));
		break;
...
jvxErrorType 
CjvxSpNSynchronize::return_hidden_interface(jvxInterfaceType tp, jvxHandle* hdl)
{
	jvxErrorType res = JVX_NO_ERROR;
	...
	case JVX_INTERFACE_CONNECTOR_MASTER_FACTORY:
		if (hdl == reinterpret_cast<jvxHandle*>(static_cast<IjvxConnectionMasterFactory*>(&sec_master)))
		{
			res = JVX_NO_ERROR;
		}
		else
