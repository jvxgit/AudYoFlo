...
CjvxAudioFileReaderTechnology::CjvxAudioFileReaderTechnology(JVX_CONSTRUCTOR_ARGUMENTS_MACRO_DECLARE):
	CjvxTemplateTechnologyVD<CjvxAudioFileReaderDevice>(JVX_CONSTRUCTOR_ARGUMENTS_MACRO_CALL)
{
...
jvxErrorType
CjvxAudioFileReaderTechnology::activate()
{
	// "Jump" over the base class implementation 
	jvxErrorType res = CjvxTemplateTechnology<CjvxAudioFileReaderDevice>::activate();
	if (res == JVX_NO_ERROR)
	{
...
jvxErrorType
CjvxAudioFileReaderTechnology::deactivate()
{
	jvxErrorType res = CjvxTemplateTechnology<CjvxAudioFileReaderDevice>::_pre_check_deactivate();
	if (res == JVX_NO_ERROR)
	{
		...
		CjvxTemplateTechnology<CjvxAudioFileReaderDevice>::deactivate();
	}
	return res;
}

jvxErrorType
CjvxAudioFileReaderTechnology::return_device(IjvxDevice* dev)
{
	// If we are here, the device was returned. We ay remove it here!
	jvxErrorType res = CjvxTemplateTechnologyVD<CjvxAudioFileReaderDevice>::return_device(dev);
	if (res == JVX_NO_ERROR)
	{
...
JVX_PROPERTIES_FORWARD_C_CALLBACK_EXECUTE_FULL(CjvxAudioFileReaderTechnology, modify_fileio)
{
...
void
CjvxAudioFileReaderTechnology::update_local_properties(jvxBool trig_host, jvxSize idSelectNew)
{
	std::string old_sel;
	jvxSize cnt;
	jvxSize idSel = jvx_bitfieldSelection2Id(
	...
	for (std::pair<IjvxDevice*, CjvxAudioFileReaderDevice*> elm : lstDevType)
	{
...
void
CjvxAudioFileReaderTechnology::trigger_close_file_core(std::pair<IjvxDevice*, CjvxAudioFileReaderDevice*> elm)
{
	jvxErrorType res = JVX_NO_ERROR;
...
jvxErrorType
CjvxAudioFileReaderTechnology::put_configuration(jvxCallManagerConfiguration* callMan, IjvxConfigProcessor* processor,
...
jvxErrorType 
CjvxAudioFileReaderTechnology::get_configuration(jvxCallManagerConfiguration* callMan,
...
jvxErrorType
CjvxAudioFileReaderTechnology::activateOneFile(
	const std::string& filename_to_be_opened,
	std::string& devSpec,
	jvxSize& idSelectNew )
{
	CjvxAudioFileReaderDevice* newDevice = nullptr;
	...
	newDevice = new CjvxAudioFileReaderDevice(
		devSpec.c_str(), false,
...
jvxErrorType 
CjvxAudioFileReaderTechnology::done_configuration()
{
	jvxErrorType res = JVX_NO_ERROR;
