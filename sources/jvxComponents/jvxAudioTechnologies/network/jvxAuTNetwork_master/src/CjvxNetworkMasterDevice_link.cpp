...
jvxErrorType 
CjvxNetworkMasterDevice::st_callback_link_report_startup_complete(jvxHandle* privateData, jvxSize ifidx)
{
	if(privateData)
	{
...
jvxErrorType 
CjvxNetworkMasterDevice::st_callback_link_provide_data_and_length( jvxHandle* privateData, jvxSize channel, char** fld, jvxSize* sz, jvxSize* offset, jvxSize ifidx, jvxHandle* add, jvxConnectionPrivateTypeEnum whatsthis)
{
	if(privateData)
	{
		CjvxNetworkMasterDevice* this_ptr = reinterpret_cast<CjvxNetworkMasterDevice*>(privateData);
...
jvxErrorType 
CjvxNetworkMasterDevice::st_callback_link_report_data_and_read( jvxHandle* privateData, jvxSize channel, char* fld, jvxSize sz, jvxSize offset, jvxSize ifidx, jvxHandle* add, jvxConnectionPrivateTypeEnum whatsthis)
{
	if(privateData)
	{
		CjvxNetworkMasterDevice* this_ptr = reinterpret_cast<CjvxNetworkMasterDevice*>(privateData);
...
jvxErrorType 
CjvxNetworkMasterDevice::st_callback_link_report_unsuccesful_outgoing( jvxHandle* privateData, jvx_connect_fail_reason reason, jvxSize ifidx)
{
	if(privateData)
	{
		CjvxNetworkMasterDevice* this_ptr = reinterpret_cast<CjvxNetworkMasterDevice*>(privateData);

		return(this_ptr->ic_callback_link_report_unsuccesful_outgoing(reason, ifidx));
	}
	return(JVX_ERROR_INVALID_ARGUMENT);
}

jvxErrorType 
CjvxNetworkMasterDevice::st_callback_link_report_connect_outgoing( jvxHandle* privateData, jvxSize ifidx)
{
	if(privateData)
	{
		CjvxNetworkMasterDevice* this_ptr = reinterpret_cast<CjvxNetworkMasterDevice*>(privateData);

		return(this_ptr->ic_callback_link_report_connect_outgoing(ifidx));
	}
	return(JVX_ERROR_INVALID_ARGUMENT);
}

jvxErrorType
CjvxNetworkMasterDevice::st_callback_link_report_disconnect_outgoing(jvxHandle* privateData, jvxSize ifidx)
{
	if (privateData)
	{
		CjvxNetworkMasterDevice* this_ptr = reinterpret_cast<CjvxNetworkMasterDevice*>(privateData);

		return(this_ptr->ic_callback_link_report_disconnect_outgoing(ifidx));
	}
	return(JVX_ERROR_INVALID_ARGUMENT);
}

jvxErrorType
CjvxNetworkMasterDevice::st_callback_link_report_shutdown_complete(jvxHandle* privateData, jvxSize ifidx)
{
	if(privateData)
	{
		CjvxNetworkMasterDevice* this_ptr = reinterpret_cast<CjvxNetworkMasterDevice*>(privateData);
...
jvxErrorType 
CjvxNetworkMasterDevice::st_callback_link_report_error(jvxHandle* privateData, jvxInt32 errCode, const char* errDescription, jvxSize ifidx)
{
	if(privateData)
	{
		CjvxNetworkMasterDevice* this_ptr = reinterpret_cast<CjvxNetworkMasterDevice*>(privateData);
...
jvxErrorType
CjvxNetworkMasterDevice::st_callback_link_report_specific(jvxHandle* privateData, jvx_connect_specific_sub_type idWhat, jvxHandle* fld, jvxSize szFld, jvxSize ifidx)
{
	if (privateData)
	{
		CjvxNetworkMasterDevice* this_ptr = reinterpret_cast<CjvxNetworkMasterDevice*>(privateData);
...
jvxErrorType 
CjvxNetworkMasterDevice::ic_callback_link_report_startup_complete(jvxSize ifidx)
{
	return(JVX_NO_ERROR);
}

jvxErrorType 
CjvxNetworkMasterDevice::ic_callback_link_provide_data_and_length(jvxSize chan, char** fldUse, 
	jvxSize* szBestFit, jvxSize* offset, jvxSize ifidx, jvxHandle* add, jvxConnectionPrivateTypeEnum whatsthis)
{
		...
		if(hdr->fam_hdr.proto_family == JVX_PROTOCOL_TYPE_AUDIO_DEVICE_LINK)
		{
			if(purpose_command == JVX_PROTOCOL_ADVLINK_SEND_DATA) 
			{
				...
				else
				{
					...
					{
						if (*szBestFit != (jvxDataFormat_size[_common_set_ldslave.theData_out.con_params.format] *
							_common_set_ldslave.theData_out.con_params.buffersize))
						{
							std::cout << "Warning, found " << *szBestFit << " bytes, expected " << (jvxDataFormat_size[_common_set_ldslave.theData_out.con_params.format] *
								_common_set_ldslave.theData_out.con_params.buffersize) << " bytes." << std::endl;
						}
						// Routing of input cannel
						*szBestFit = JVX_MIN(*szBestFit, (jvxDataFormat_size[_common_set_ldslave.theData_out.con_params.format] * 
							_common_set_ldslave.theData_out.con_params.buffersize));
...
jvxErrorType 
CjvxNetworkMasterDevice::ic_callback_link_report_data_and_read(jvxSize, char*, jvxSize sz, jvxSize offset,  jvxSize ifidx, jvxHandle* priv, jvxConnectionPrivateTypeEnum whatsthis)
{
	jvxSize j;
		...
		if(header->fam_hdr.proto_family == JVX_PROTOCOL_TYPE_AUDIO_DEVICE_LINK)
		{
			...
			if(purpose_command != JVX_PROTOCOL_ADVLINK_SEND_DATA)
			{
				...
				if(inConnection.channel.inUse)
				{
					...
					res = procedeStateMachine(hdr);
				}
				else
				{
					// We end up here only if the stop command has just been sent and
					// the external device has not yet responded. In this case, we need to skip this buffer
					// and leave this function since the response to the stop command must be handled within this thread
					// here!
					JVX_LOG_TO_FILE(JVX_PROFILE_EVENT_CHANNEL_BUSY, 0, 0, inProcessing.logId, 0);

					genNetworkMaster_device::integrateiplink.properties_running.numberLostFrames.value++;
						...
						(inConnection.localStateMachineConnect == JVX_STATE_PROCESSING))
					{
						...
						if(inConnection.oneChannelInOut.channelMask == inProcessing.channelMaskTarget)
						{
							...
							else
							{
								...
								genNetworkMaster_device::integrateiplink.properties_running.numberLostFramesRemote.value += delta;
								if(delta != 0)
								{
							...
							if (resM == JVX_TRY_LOCK_MUTEX_SUCCESS)
							{
								...
								for (j = 0; j < _common_set_ldslave.theData_in->con_params.number_channels; j++)
								{
									...
									*/
									...
									res = jvx_connect_client_send_st(this->socketClientIf, inProcessing.tx_fields[inProcessing.idSendOut][j], &szFld, NULL, JVX_CONNECT_PRIVATE_ARG_TYPE_NONE);
									...
									*/
#else
									res = jvx_connect_client_collect_st(this->socketClientIf, inProcessing.tx_fields[inProcessing.idSendOut][j], 
										&szFld, NULL, JVX_CONNECT_PRIVATE_ARG_TYPE_SOCKET_PCAP_TARGET);
									assert(res == JVX_NO_ERROR);
#endif
								...
								res = jvx_connect_client_transmit_st(this->socketClientIf);
								assert(res == JVX_NO_ERROR);
							...
							else
							{
								...
								genNetworkMaster_device::integrateiplink.properties_running.numberUnsentFrames.value++;
							}
...
jvxErrorType 
CjvxNetworkMasterDevice::ic_callback_link_report_unsuccesful_outgoing( jvx_connect_fail_reason reason, jvxSize ifidx)
{
	return(JVX_ERROR_UNSUPPORTED);
}

jvxErrorType 
CjvxNetworkMasterDevice::ic_callback_link_report_connect_outgoing( jvxSize ifidx)
{
	...
	theHeader->paketsize = sizeof(jvxProtocolHeader);
	// JVX_SIZE_INT32(inConnection.fldCommunicateHandshake_out.sz);
	...
	theHeader->paketsize = JVX_SIZE_UINT32(sz - inConnection.fldPrepend);

	jvx_message_queue_add_message_to_queue(inConnection.mQueue_out, 0, inConnection.fldCommunicateHandshake_out.ptr, sz, NULL);

	genNetworkMaster_device::integrateiplink.properties_running.connectionStatus.value.selection() = 0x8;
...
jvxErrorType
CjvxNetworkMasterDevice::ic_callback_link_report_disconnect_outgoing(jvxSize ifidx)
{
...
jvxErrorType
CjvxNetworkMasterDevice::ic_callback_link_report_shutdown_complete(jvxSize ifidx)
{
	jvxErrorType resL;
...
jvxErrorType 
CjvxNetworkMasterDevice::ic_callback_link_report_error(jvxInt32 errCode, const char* errDescription, jvxSize ifidx)
{
...
jvxErrorType
CjvxNetworkMasterDevice::ic_callback_link_report_specific(jvx_connect_specific_sub_type idWhat, jvxHandle* fld, jvxSize szFld, jvxSize ifidx)
{
	return(JVX_NO_ERROR);
}
