...
CjvxNetworkSlaveDevice::prepare_chain_master(JVX_CONNECTION_FEEDBACK_TYPE(fdb))
{
	...
	_common_set_ldslave.theData_out.con_data.number_buffers = genNetworkSlave_device::properties_active.numberBuffersDataXChange.value;
	...
	if (res == JVX_NO_ERROR)
	{
		...
		res = jvx_connect_server_start_queue_st(this->socketServerIf, socketServerChannel, qSize, _common_set_ldslave.theData_in->con_params.number_channels);
		assert(res == JVX_NO_ERROR);
#endif
	}
