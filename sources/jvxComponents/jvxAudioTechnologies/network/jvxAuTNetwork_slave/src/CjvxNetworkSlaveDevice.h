...
class CjvxNetworkSlaveTechnology;

class CjvxNetworkSlaveDevice: public CjvxAudioDevice, 
...
private:
	...
	typedef enum
	{
		/*
			...
			* 3: Status is active
			* 4: Connection ready (starting May occur immediately)
			* 5: In processing
			* 6: Shutdown in progress
			*/
		JVX_NETWORK_SLAVE_CONNECTION_OFF,
		JVX_NETWORK_SLAVE_CONNECTION_LISTENING,
		JVX_NETWORK_SLAVE_CONNECTION_CONNECTED,
		JVX_NETWORK_SLAVE_CONNECTION_ACTIVE,
		JVX_NETWORK_SLAVE_CONNECTION_READY,
		JVX_NETWORK_SLAVE_CONNECTION_PROCESSING,
		JVX_NETWORK_SLAVE_CONNECTION_SHUTDOWN
	} jvxNetSlave_connState;
	jvxNetSlave_connState connectionState;

	CjvxNetworkSlaveTechnology* parentRef;
	jvx_connect_server_if* socketServerIf;
	jvxSize socketServerChannel;
	jvxSize idIface;
	
	jvx_connect_server_callback_type report_link_cbs;
	...
	jvxBool forceDown;
	jvxBool connectionEstablished;
public:

	CjvxNetworkSlaveDevice(JVX_CONSTRUCTOR_ARGUMENTS_MACRO_DECLARE);
	~CjvxNetworkSlaveDevice();
    ...
    */
	...
	void setReferences(CjvxNetworkSlaveTechnology* parentRef, jvxSize idIf, jvx_connect_socket_type tp);
	...
	jvxErrorType startListen(jvxHandle*, jvxConnectionPrivateTypeEnum whatsthis);
	jvxErrorType stopListen();
	jvxErrorType procedeStateMachine(jvxProtocolHeader* hdr, jvxHandle* priv, jvxConnectionPrivateTypeEnum whatsthis);
	...
	static jvxErrorType st_callback_report_denied_incoming(jvxHandle* privateData, const char* description, jvxSize ifidx);
	static jvxErrorType st_callback_report_connect_incoming(jvxHandle* privateData, jvxSize channelId, const char* description, jvxSize ifidx);
	static jvxErrorType st_callback_report_disconnect_incoming(jvxHandle* privateData, jvxSize channelId, jvxSize ifidx); 
	...
	jvxErrorType ic_callback_report_denied_incoming(const char* description, jvxSize ifidx);
	jvxErrorType ic_callback_report_connect_incoming(jvxSize channelId, const char* description, jvxSize ifidx);
	jvxErrorType ic_callback_report_disconnect_incoming(jvxSize channelId, jvxSize ifidx);
	...
	// Convert selections etc in specified parameters to exposd parameters
	void update_exposed_properties_no_lock();

	// Store the current properties into the internal representation
	void propsToInternalValue_no_lock();
	void internalValueToProps_no_lock();
