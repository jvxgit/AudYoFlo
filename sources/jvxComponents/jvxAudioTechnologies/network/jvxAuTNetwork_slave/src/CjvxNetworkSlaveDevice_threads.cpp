...
jvxErrorType 
CjvxNetworkSlaveDevice::st_callback_thread_startup(jvxHandle* privateData, jvxInt64 timestamp_us)
{
	if(privateData)
	{
...
jvxErrorType 
CjvxNetworkSlaveDevice::st_callback_thread_timer_expired(jvxHandle* privateData, jvxInt64 timestamp_us, jvxSize* delta_ms)
{
	if(privateData)
	{
		CjvxNetworkSlaveDevice* this_ptr = reinterpret_cast<CjvxNetworkSlaveDevice*>(privateData);
...
jvxErrorType 
CjvxNetworkSlaveDevice::st_callback_thread_wokeup(jvxHandle* privateData, jvxInt64 timestamp_us, jvxSize* delta_ms)
{
	if(privateData)
	{
		CjvxNetworkSlaveDevice* this_ptr = reinterpret_cast<CjvxNetworkSlaveDevice*>(privateData);
...
jvxErrorType 
CjvxNetworkSlaveDevice::st_callback_thread_stopped(jvxHandle* privateData, jvxInt64 timestamp_us)
{
	if(privateData)
	{
		CjvxNetworkSlaveDevice* this_ptr = reinterpret_cast<CjvxNetworkSlaveDevice*>(privateData);
...
jvxErrorType 
CjvxNetworkSlaveDevice::ic_callback_thread_startup(jvxInt64 timestamp_us)
{
	return(JVX_NO_ERROR);
}

jvxErrorType 
CjvxNetworkSlaveDevice::ic_callback_thread_timer_expired(jvxInt64 timestamp_us)
{
	return(JVX_NO_ERROR);
}

jvxErrorType 
CjvxNetworkSlaveDevice::ic_callback_thread_wokeup(jvxInt64 timestamp_us)
{
	jvxSize i;
	...
	while(inProcessing.fillHeight > 0)	
	{
		...
		if (rr == JVX_TRY_LOCK_MUTEX_SUCCESS)
		{
			if (HjvxDataLogger_Otf::status == JVX_STATE_PROCESSING)
			{
				...
				if (dataLogging.fldWrite_rawaudio)
				{
					...
					genNetworkSlave_device::timinglogtofile.properties_logtofile.fillHeightBuffer.value.val() = tmp * 100;
				}
			}
			else
			{
		...
		genNetworkSlave_device::profiling.deltaT_converters.value = (ALPHA)* genNetworkSlave_device::profiling.deltaT_converters.value +
			(1 - ALPHA)*((jvxData)(t1 - t0));

		genNetworkSlave_device::profiling.deltaT_processing.value = (ALPHA)* genNetworkSlave_device::profiling.deltaT_processing.value +
			(1 - ALPHA)*((jvxData)(t2 - t1));

		if (JVX_CHECK_SIZE_SELECTED(converters.timeOld))
		{
			genNetworkSlave_device::profiling.deltaT_buffers.value = (ALPHA)* genNetworkSlave_device::profiling.deltaT_buffers.value +
				(1 - ALPHA)*((jvxData)(t0 - converters.timeOld));

			genNetworkSlave_device::integrateiplink.properties_running.profile_load_convert.value =
				ALPHA * genNetworkSlave_device::integrateiplink.properties_running.profile_load_convert.value +
				(1 - ALPHA)* (genNetworkSlave_device::profiling.deltaT_converters.value / converters.deltaT_theo * 100.0);
			genNetworkSlave_device::integrateiplink.properties_running.profile_load_process.value =
				ALPHA * genNetworkSlave_device::integrateiplink.properties_running.profile_load_process.value +
				(1 - ALPHA)* (genNetworkSlave_device::profiling.deltaT_processing.value / converters.deltaT_theo * 100.0);
		}
...
jvxErrorType 
CjvxNetworkSlaveDevice::ic_callback_thread_stopped(jvxInt64 timestamp_us)
{
	return(JVX_NO_ERROR);
}
