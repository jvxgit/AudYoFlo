...
jvxErrorType
CjvxAudioFFMpegWriterTechnology::put_configuration(jvxCallManagerConfiguration* callMan, IjvxConfigProcessor* processor,
	jvxHandle* sectionToContainAllSubsectionsForMe, const char* filename, jvxInt32 lineno )
{
	...
	if (_common_set_min.theState == JVX_STATE_ACTIVE)
	{
		...
		genFFMpegWriter_technology::put_configuration_all(callMan,
			processor, sectionToContainAllSubsectionsForMe,
			&warns);

		/*
		selId = jvx_bitfieldSelection2Id(genFFMpegWriter_technology::file_params.device_lists.value.selection,
			genFFMpegWriter_technology::file_params.device_lists.value.entries.size());
		if (selId < genFFMpegWriter_technology::file_params.device_lists.value.entries.size())
		{
			selectedFile = genFFMpegWriter_technology::file_params.device_lists.value.entries[selId];
		}

		for (i = 0; i < genFFMpegWriter_technology::file_params.device_lists.value.entries.size(); i++)
		{
			fNamesOnConfig.push_back(genFFMpegWriter_technology::file_params.device_lists.value.entries[i]);
		}
		genFFMpegWriter_technology::file_params.device_lists.value.entries.clear();
		jvx_bitFClear(genFFMpegWriter_technology::file_params.device_lists.value.selection);
		*/
		genFFMpegWriter_technology::file_params.device_lists.value.entries.clear();
...
jvxErrorType 
CjvxAudioFFMpegWriterTechnology::get_configuration(jvxCallManagerConfiguration* callMan,
	IjvxConfigProcessor* processor, jvxHandle* sectionWhereToAddAllSubsections)
{
	
	genFFMpegWriter_technology::get_configuration_all(callMan,
...
jvxErrorType
CjvxAudioFFMpegWriterTechnology::done_configuration()
{
	jvxErrorType res = JVX_NO_ERROR;
