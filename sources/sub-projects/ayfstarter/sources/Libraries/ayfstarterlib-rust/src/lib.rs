...
#[no_mangle]
pub fn ayf_starter_init(hdl: *mut ayf_starter, bsize: JvxSize) -> JvxDspBaseErrorType {
    let hdl_ = unsafe { &mut *hdl };
    if hdl_.prv.is_none() {
        hdl_.prm_init.bsize = bsize;
        hdl_.prm_async.volume = 1.0;
        hdl_.prm_async.runorc = true;
...
#[no_mangle]
pub fn ayf_starter_prepare(hdl: *mut ayf_starter) -> JvxDspBaseErrorType {
    let hdl_ = unsafe { &mut *hdl };
    if hdl_.prv.is_none() {
...
#[no_mangle]
pub fn ayf_starter_process(hdl: *mut ayf_starter, inputs: *const *const JvxData, outputs: *const *mut JvxData, num_channels_in: JvxSize, num_channels_out: JvxSize, buffer_size: JvxSize) -> JvxDspBaseErrorType {
    let hdl_ = unsafe { &mut *hdl };
...
#[no_mangle]
pub fn ayf_starter_postprocess(hdl: *mut ayf_starter) -> JvxDspBaseErrorType {
    let hdl_ = unsafe { &mut *hdl };
    if hdl_.prv.is_some() {
        hdl_.prv = None;
        return JvxDspBaseErrorType::JVX_NO_ERROR;
...
#[no_mangle]
pub fn ayf_starter_update(hdl: *mut ayf_starter, operation: u16, want_to_set: bool) -> JvxDspBaseErrorType {
    let hdl_ = unsafe { &mut *hdl };
    if let Some(ref mut prv) = hdl_.prv {
