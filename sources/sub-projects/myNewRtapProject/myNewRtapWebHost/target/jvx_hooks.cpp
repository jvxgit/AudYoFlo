	...
	jvxErrorType jvx_configure_factoryhost_features(configureFactoryHost_features* features)
{
	if (features)
	{
		configureHost_features* featuresA = NULL;
		features->request_specialization(reinterpret_cast<jvxHandle**>(&featuresA), JVX_HOST_IMPLEMENTATION_HOST);
		if (featuresA)
		{
			configureHost_features_local(featuresA);
		}
	}
	return(JVX_NO_ERROR);
}
