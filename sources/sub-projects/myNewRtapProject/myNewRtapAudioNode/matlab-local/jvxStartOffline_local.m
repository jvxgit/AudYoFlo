function [ jvx_handle ] = jvxStartOffline_local( jvx_bsize_in, jvx_srate_in, jvx_num_in, jvx_format_in, jvx_bsize_out, jvx_srate_out, jvx_num_out, jvx_format_out, jvx_is_offline, precision, jvx_add_hints)
    ...
    if(jvx_bsize_in ~= jvx_bsize_out)
        error('Input block size must be identical to output block size');
    end
    
    if(jvx_format_in ~= jvx_format_out)
        error('Input format must be identical to output format');
    end
    
    % Initialize your algorithm here
    addpath ( [fileparts(mfilename('fullpath')) filesep '..' filesep '..' filesep 'myNewRtapLibrary-matlab'] );
    jvx_handle.myNewProject = jvx_myNewRtapLibrary.jvx_myNewProject;
