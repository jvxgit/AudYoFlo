...
if not exist %ABS_PATH_INSTALL%\..\build-debug md %ABS_PATH_INSTALL%\..\build-debug

REM Specify path reference to SDK folder
set /p sdkpath="--> Specify location of debug build of SDK folder [%JVX_SDK_PATH%]:"
...
%PTSED% s@JVX_RPL_GENERATOR_TOKEN@%CMAKE_TOKEN%@g standalone\build-debug\compile.bat > ___j__v__x__tmp.bat || ( goto :error_sed_failed )
%PTSED% s@JVX_RPL_SDK_PATH@%JVX_SDK_PATH%@g ___j__v__x__tmp.bat > ..\build-debug\compile.bat || ( goto :error_sed_failed )
