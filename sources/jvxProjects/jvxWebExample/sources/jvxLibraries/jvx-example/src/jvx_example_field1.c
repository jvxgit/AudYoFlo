...
#define JVX_NUM_VALUES_PRAMIRQ0 4096
...
{
	int file_hdl;
	jvx_example_field1_sync_str sync_copy;
} jvx_example_field1_private;

jvxDspBaseErrorType
jvx_example_field1_initCfg(jvx_example_field1* theHandle)
{
	theHandle->prv = NULL;
	theHandle->prmInit.dev_target = "/dev/jvx_example_field1";
...
jvxDspBaseErrorType
jvx_example_field1_init(jvx_example_field1* theHandle)
{
	jvx_example_field1_private* prv = NULL;
	jvxDspBaseErrorType res = JVX_DSP_ERROR_WRONG_STATE;
	jvxSize nBytes = 0;
	jvxSize i;
	if (theHandle->prv == NULL)
	{
		JVX_DSP_SAFE_ALLOCATE_OBJECT_Z(prv, jvx_example_field1_private);

		prv->sync_copy.fld = NULL;
		prv->sync_copy.fld_num = 0;
		prv->sync_copy.fld_sz = 0;

		prv->sync_copy.fld_num = JVX_NUM_VALUES_PRAMIRQ0;
		...
		theHandle->prv = prv;
		res = jvx_example_field1_update(theHandle, c_false, JVX_DSP_UPDATE_SYNC);
...
jvxDspBaseErrorType
jvx_example_field1_update(jvx_example_field1* theHandle, jvxCBool do_set, jvxUInt16 what)
{
    jvxDspBaseErrorType res = JVX_DSP_ERROR_WRONG_STATE;
	if (theHandle->prv)
	{
		jvx_example_field1_private* prv = (jvx_example_field1_private*)theHandle->prv;
		if (do_set)
		{
			switch (what)
			{
...
jvxDspBaseErrorType
jvx_example_field1_terminate(jvx_example_field1* theHandle)
{
	jvxSize nBytes = 0;
#ifdef JVX_OS_LINUX
	int reti;
...
/*
jvxDspBaseErrorType 
jvx_example_field1_trigger_read(jvx_example_field1* theHandle)
{
	jvxDspBaseErrorType res = JVX_DSP_ERROR_WRONG_STATE;
	jvxInt32 val = 0;
	if (theHandle->prv)
