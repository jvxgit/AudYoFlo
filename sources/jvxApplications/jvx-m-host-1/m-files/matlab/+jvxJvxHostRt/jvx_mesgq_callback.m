...
if(bitand(eventmask, bitshift(1, vTestChain)))
    [suc, retVal, ss] = jvx_host_call_global('test_connection_master', param1);
    if(suc)
        if(retVal)
            display(['Testing signal chain ok.']);
        else
          display(['Testing signal chain failed, reason: ' ss]);
        end
    else
        display(['Function to test signal chain failed, reason: ' retVal]);
    end
