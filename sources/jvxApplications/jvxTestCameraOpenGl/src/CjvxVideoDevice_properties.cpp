...
void 
CjvxVideoDevice::scanProperties(IMFSourceReader* reader)
{
	HRESULT hr;
	...
	while (nativeTypeErrorCode == S_OK && besterror)
	{
		...
		oneForm.form = JVX_VIDEO_FORMAT_UNKNOWN;
		oneForm.height = 0;
		oneForm.width = 0;
		oneForm.id = count;
		oneForm.fps_min = 0;
...
#endif
			oneForm.form = JVX_VIDEO_FORMAT_RGB_24;
		}		
		...
		if (oneForm.form != JVX_VIDEO_FORMAT_UNKNOWN)
		{
#ifdef VERBOSE_1
			std::cout << "Mode #" << lstModes.size() << ": " << oneForm.width << "x" << oneForm.height << ", " << std::flush;
			switch (oneForm.form)
			{
			case JVX_VIDEO_FORMAT_RGB_24:
