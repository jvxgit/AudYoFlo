...
stringAssignmentProperties::stringAssignmentProperties(jvxComponentIdentification tp, jvxSize idx, jvxPropertyCategoryType category, jvxUInt64 allowStateMask,
...
void
stringAssignmentProperties::init()
{
...
void
stringAssignmentProperties::updateWindow()
{
...
//		IjvxHost* theLink;
	...
	jvx_pullPropertyString(entry, propertyParams.theObj, propertyParams.theProps, propertyParams.handleIdx, propertyParams.category, callGate);
	lineEdit_value->setText(entry.c_str());
}

void
stringAssignmentProperties::textEdited()
{
	jvxCallManagerProperties callGate;
	std::string entry = lineEdit_value->text().toLatin1().constData();;
	jvx_pushPullPropertyString(entry, propertyParams.theObj, propertyParams.theProps, propertyParams.handleIdx, propertyParams.category, 
		callGate);
	updateWindow();
}
