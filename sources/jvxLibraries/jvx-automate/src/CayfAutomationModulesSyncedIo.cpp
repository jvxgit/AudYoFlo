...
namespace CayfAutomationModules
{
	...
	// ==================================================================================================
	// Check if this part must be connected!!
	void CayfAutomationModulesSyncedIoPrimary::try_connect(
		...
		std::list<ayfOneConnectedProcess>& connectedProcesses)
	{
		...
		{
			jvxDataConnectionRuleParameters params(false, false, true, supportNodeRuntime.driveSupportNodeChain.misc.dbgOut, true);
			params.preventStoreConfig = true;
			std::string chainName = supportNodeRuntime.driveSupportNodeChain.chainNamePrefix +
			...
			res = con->create_connection_rule(chainName.c_str(), &theDataConnectionDefRule_id,
				&params, supportNodeRuntime.driveSupportNodeChain.misc.connectionCategory);
			if (res == JVX_NO_ERROR)
			{
				jvxSize cnt = 0;
				std::string bridgename = "bridge_" + jvx_size2String(cnt);
				...
				if ((res == JVX_NO_ERROR) && (theDataConnectionDefRuleHdl))
				{
					...
					res = theDataConnectionDefRuleHdl->specify_master(tp_reg,
						"*", supportNodeRuntime.driveSupportNodeChain.masterNm.c_str());
					assert(res == JVX_NO_ERROR);

					res = theDataConnectionDefRuleHdl->add_bridge_specification(
						tp_reg,
						"*", supportNodeRuntime.driveSupportNodeChain.oconMasterNm.c_str(),
						supportNodeRuntime.cpId,
						"*", supportNodeRuntime.driveSupportNodeChain.iconNmConnect.c_str(), bridgename.c_str(), false, false);
					assert(res == JVX_NO_ERROR);
					...
					res = theDataConnectionDefRuleHdl->add_bridge_specification(
						supportNodeRuntime.cpId,
						"*", supportNodeRuntime.driveSupportNodeChain.oconNmConnect.c_str(),
						tp_reg,
						"*", supportNodeRuntime.driveSupportNodeChain.iconMasterNm.c_str(), bridgename.c_str(), false, false);
					assert(res == JVX_NO_ERROR);
					theDataConnectionDefRuleHdl->mark_rule_default();
