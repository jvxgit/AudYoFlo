...
jvxErrorType
CjvxSubQueue::hook_test_negotiate(jvxLinkDataDescriptor* proposed JVX_CONNECTION_FEEDBACK_TYPE_A(fdb))
{
...
jvxErrorType
CjvxSubQueue::hook_test_accept(jvxLinkDataDescriptor* dataIn JVX_CONNECTION_FEEDBACK_TYPE_A(fdb))
{
...
jvxErrorType
CjvxSubQueue::hook_test_update(jvxLinkDataDescriptor* dataIn  JVX_CONNECTION_FEEDBACK_TYPE_A(fdb))
{
...
jvxErrorType
CjvxSubQueue::hook_forward(jvxLinkDataTransferType tp, jvxHandle* data JVX_CONNECTION_FEEDBACK_TYPE_A(fdb))
{
	return CjvxBareNode1io_rearrange::_transfer_forward_ocon(tp, data JVX_CONNECTION_FEEDBACK_CALL_A(fdb));
}

jvxErrorType
CjvxSubQueue::hook_check_is_ready(jvxBool* is_ready, jvxApiString* astr)
{
	return JVX_NO_ERROR;
}

jvxErrorType
CjvxSubQueue::hook_prepare(JVX_CONNECTION_FEEDBACK_TYPE(fdb))
{
...
jvxErrorType
CjvxSubQueue::hook_postprocess(JVX_CONNECTION_FEEDBACK_TYPE(fdb))
{
	return _postprocess_connect_icon(true JVX_CONNECTION_FEEDBACK_CALL_A(fdb));
}

jvxErrorType
CjvxSubQueue::hook_start(JVX_CONNECTION_FEEDBACK_TYPE(fdb))
{
...
jvxErrorType
CjvxSubQueue::hook_stop(JVX_CONNECTION_FEEDBACK_TYPE(fdb))
{
	return _stop_connect_icon(true, nullptr JVX_CONNECTION_FEEDBACK_CALL_A(fdb));
}

jvxErrorType
CjvxSubQueue::hook_process_start(
...
jvxErrorType
CjvxSubQueue::hook_process(jvxSize mt_mask, jvxSize idx_stage)
{
	jvxErrorType res = JVX_NO_ERROR;
	if (_common_set_ldslave.ocon)
	{
...
jvxErrorType
CjvxSubQueue::hook_process_stop(
	jvxSize idx_stage,
	jvxBool shift_fwd,
	jvxSize tobeAccessedByStage,
