			...
			if ((fldSize - offset) >= szPack)
			{
                ...
                case JVX_PROTOCOL_ADVLINK_SEND_DATA:
                    if (myPriv->state == JVX_AUDIOSTACK_STATE_PROCESSING)
                    {
                        ...
                        {
                            resL = myPriv->init.common.data_callback(myPriv->init.common.prv_callbacks, dataDir, header->paketsize - (sizeof(jvxAdvProtocolDataP) - sizeof(jvxUInt8)), data->adv_data_header.channel_mask, data->adv_data_header.sequence_id);
                        }
                    }
                    break;
				}
