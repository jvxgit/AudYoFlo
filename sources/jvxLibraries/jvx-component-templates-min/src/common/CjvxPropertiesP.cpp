...
CjvxProperties::CjvxProperties(const std::string& moduleName, IjvxObject* objRef ):
	theObjReference(objRef)
{
...
jvxErrorType
CjvxProperties::number_properties(jvxCallManagerProperties * callGate, jvxSize * num)
{
	jvxSize idx;
	jvxSize numL = 0;
	...
	if (num)
	{
		*num = 0;
		...
		{
			res = _common_set_properties.registeredSubmodules[idx].propRef->number_properties(
				callGate, &numL);
			if (res == JVX_NO_ERROR)
			{
				*num += numL;
	...
	jvxCheckAccessEnum accEnum)
{
	if (theObjReference)
	{
		theObjReference->check_access(callGate);
...
jvxErrorType
CjvxProperties::get_revision(jvxCallManagerProperties * callGate, jvxSize * revision)
{
	if (revision)
...
jvxErrorType
CjvxProperties::lock_properties(jvxCallManagerProperties * callGate)
{
...
jvxErrorType
CjvxProperties::trylock_properties(jvxCallManagerProperties * callGate)
{
	jvxErrorType res = _check_access_property_obj(callGate, jvxCheckAccessEnum::JVX_CHECK_ACCESS_LOCK);
	if (res != JVX_NO_ERROR)
		return res;
...
jvxErrorType
CjvxProperties::unlock_properties(jvxCallManagerProperties * callGate)
{
	jvxErrorType res = _check_access_property_obj(callGate, jvxCheckAccessEnum::JVX_CHECK_ACCESS_LOCK);
	if (res != JVX_NO_ERROR)
		return res;
...
jvxErrorType
CjvxProperties::description_property(
	...
	const jvx::propertyAddress::IjvxPropertyAddress& addrProp)
{
	...
	{
		res = _forward_submodules(callGate, addresses, [&](const jvx::propertyAddress::IjvxPropertyAddress& addr, jvxSize i)
			{
				jvxErrorType resL = JVX_NO_ERROR;
				resL = _common_set_properties.registeredSubmodules[i].propRef->description_property(callGate, descr, addr);
				return resL;
			}, [&](jvxSize i) {
				if (descrs.dMin)
				{
					descrs.dMin->globalIdx += _common_set_properties.registeredSubmodules[i].offsetShift;
				}
				if (descrs.dFull)
				{
					std::string tag;
					if (!_common_set_properties.registeredSubmodules[i].propDescriptionPrefix.empty())
					{
						tag = jvx_makePathExpr(_common_set_properties.registeredSubmodules[i].propDescriptorTag,
	...
	const jvx::propertyDetail::CjvxTranferDetail& detail)
{
		...
		if (selection->references.fld)
		{
			...
			case JVX_DATAFORMAT_STRING:
				if (numElementsLocal == 1)
				{
					...
					{
						jvx_oneSelectionOption_cpp* selL = (jvx_oneSelectionOption_cpp*)selection->references.fld;
			...
			case JVX_DATAFORMAT_SELECTION_LIST:
				if (numElementsLocal == 1)
				{
					...
					if (tune.contentOnly)
					{
						if (_common_set_properties.reportIfNoChange)
						{
							if (selL->selection == selLIn->bitFieldSelected)
							{
								callGate->on_set.modification_impact = false;
							}
						}
			...
			case JVX_DATAFORMAT_HANDLE:
				...
				break;
			case JVX_DATAFORMAT_PROPERTY_EXTENDER_INTERFACE:
	...
	else
	{
		...
		{
			jvxErrorType resL = JVX_NO_ERROR;
			resL = _common_set_properties.registeredSubmodules[i].propRef->set_property(
				callGate,
				rawData,
				/*
				fld,
	...
	const jvx::propertyDetail::CjvxTranferDetail& detail)
{
	...
	else
	{
		res = _forward_submodules(callGate, addresses, [&](const jvx::propertyAddress::IjvxPropertyAddress& addr, jvxSize i)
			{
				jvxErrorType resL = JVX_NO_ERROR;
				resL = _common_set_properties.registeredSubmodules[i].propRef->get_property(
					callGate,
					rawData,
					addr, detail);
				return resL;
...
jvxErrorType
CjvxProperties::install_property_reference(
	jvxCallManagerProperties * callGate,
	const jvx::propertyRawPointerType::IjvxRawPointerType& rawData,
	const jvx::propertyAddress::IjvxPropertyAddress& ident)
{
	...
	else
	{
		res = _forward_submodules(callGate, addresses, [&](const jvx::propertyAddress::IjvxPropertyAddress& addr, jvxSize i)
		{
			 jvxErrorType resL = _common_set_properties.registeredSubmodules[i].propRef->install_property_reference(
				callGate,
				rawData, 
				addr);
			return resL;
...
jvxErrorType
CjvxProperties::uninstall_property_reference(
	jvxCallManagerProperties * callGate,
	const jvx::propertyRawPointerType::IjvxRawPointerType& rawData,
	const jvx::propertyAddress::IjvxPropertyAddress &ident)
{
	...
	else
	{
		res = _forward_submodules(callGate, addresses, [&](const jvx::propertyAddress::IjvxPropertyAddress& addr, jvxSize i)
		{
			jvxErrorType resL = _common_set_properties.registeredSubmodules[i].propRef->uninstall_property_reference(
				callGate,
				rawData, 
				addr);
			return resL;
...
jvxErrorType
CjvxProperties::get_property_extended_info(jvxCallManagerProperties * callGate, 
	jvxHandle * fld,
	jvxSize requestId,
	const jvx::propertyAddress::IjvxPropertyAddress& ident)
{
	...
	else
	{
		res = _forward_submodules(callGate, addresses, [&](const jvx::propertyAddress::IjvxPropertyAddress& addr, jvxSize i)
			{
				jvxErrorType resL = JVX_NO_ERROR;
				resL = _common_set_properties.registeredSubmodules[i].propRef->get_property_extended_info(
...
jvxErrorType
CjvxProperties::get_meta_flags(jvxCallManagerProperties * callGate,	
	...
	const jvx::propertyAddress::IjvxPropertyAddress& ident)
{
	...
	else
	{
		res = _forward_submodules(callGate, addresses, [&](const jvx::propertyAddress::IjvxPropertyAddress& addr, jvxSize i)
			{
				jvxErrorType resL = JVX_NO_ERROR;
				resL = _common_set_properties.registeredSubmodules[i].propRef->get_meta_flags(
					callGate,
					access_flags,
					mode_flags,
					ident);
...
jvxErrorType
CjvxProperties::set_meta_flags(jvxCallManagerProperties * callGate,	
	jvxAccessRightFlags_rwcd * access_flags,
	jvxConfigModeFlags * mode_flags,
	const jvx::propertyAddress::IjvxPropertyAddress& ident)
{
	...
	else
	{
		res = _forward_submodules(callGate, addresses, [&](const jvx::propertyAddress::IjvxPropertyAddress& addr, jvxSize i)
			{
				jvxErrorType resL = JVX_NO_ERROR;
				resL = _common_set_properties.registeredSubmodules[i].propRef->set_meta_flags(
					callGate,
					access_flags,
					mode_flags,
					ident);
...
jvxErrorType
CjvxProperties::_initialize_properties(IjvxObject * hObj, bool useLock)
{
...
)
{
	...
	if (selection == _common_set_properties.registeredProperties.end())
	{
		...
		{
		case JVX_DATAFORMAT_CALLBACK:
			elm.references.fld_prop_cb = (jvxCallbackPrivate**)references.fld;
			break;
			break;
		default:
...
jvxErrorType
CjvxProperties::_update_property(jvxHandle * fld, jvxSize numberElements, jvxPropertyCategoryType cat, jvxSize idProp)
{
	jvxErrorType res = JVX_NO_ERROR;
	std::list<oneProperty>::iterator selection;
...
jvxErrorType
CjvxProperties::request_takeover_property(
	jvxCallManagerProperties * callGate,
...
jvxErrorType
CjvxProperties::_register_sub_module(IjvxProperties* theProps, IjvxObject* theObj, jvxSize offsetShft, std::string propDescriptorTag, std::string propDescriptionPrefix)
{
	jvxErrorType res = JVX_NO_ERROR;
	_lock_properties_local();

	std::vector<oneSubModule>::iterator elm = _common_set_properties.registeredSubmodules.begin();
	...
	if (elm == _common_set_properties.registeredSubmodules.end())
	{
		...
		std::sort(_common_set_properties.registeredSubmodules.begin(), _common_set_properties.registeredSubmodules.end());
	}
	else
	{
		res = JVX_ERROR_DUPLICATE_ENTRY;
...
jvxSize CjvxProperties::_update_max_prop_id()
{
	...
	if (_common_set_properties.registeredSubmodules.size() > 0)
	{
		...
		jvxSize offs = _common_set_properties.registeredSubmodules[_common_set_properties.registeredSubmodules.size() - 1].offsetShift;
		jvxSize numP = 0;
		_common_set_properties.registeredSubmodules[_common_set_properties.registeredSubmodules.size() - 1].propRef->number_properties(&callGate, &numP);
...
jvxErrorType
CjvxProperties::reset_property_group(jvxCallManagerProperties * callGate)
{
	_common_set_properties.inPropertyGroupCnt = 0;
	return JVX_NO_ERROR;
}
...
jvxErrorType
CjvxProperties::start_property_group(jvxCallManagerProperties * callGate)
{
...
jvxErrorType
CjvxProperties::status_property_group(
	jvxCallManagerProperties * callGate,
	jvxSize * startCnt)
{
	if (startCnt)
...
jvxErrorType
CjvxProperties::stop_property_group(jvxCallManagerProperties * callGate)
{
	...
	jvxSize& globIdx, jvxPropertyCategoryType& cat)
{
	...
	res = description_property(callGate, theDescr, ident);
	if (res == JVX_NO_ERROR)
	{
...
CjvxProperties::_forward_submodules(jvxCallManagerProperties* callGate,
	...
	std::function < void(jvxSize i)> func_post_success)
{
	...
	for (i = 0; i < _common_set_properties.registeredSubmodules.size(); i++)
	{
		...
		else if (addresses.addrDescr)
		{
			...
			resL = addresses.prepDescr(addrDescrCp, _common_set_properties.registeredSubmodules[i].propDescriptorTag);
			if (resL == JVX_NO_ERROR)
			{
				res = func(addrDescrCp, i);
		...
		else if (addresses.addrIdx)
		{
			...
			resL = addresses.prepIdx(addrIdxCp, _common_set_properties.registeredSubmodules[i].offsetShift);
			if (resL == JVX_NO_ERROR)
			{
				res = func(addrIdxCp, i);
	...
	jvxPropertyCallbackPurpose purp)
{
	...
	if (hookname != "")
	{
		...
		if (selectionC != _common_set_properties.registeredCallbacks.end())
		{
			if (selectionC->theCallback)
			{
				...
				if (res != JVX_NO_ERROR)
				{
					...
					{
						std::cout << "Warning <" << _common_set_properties.moduleReference << ">: shortcut callback <" << hookname
		...
		else
		{
			if (_common_set_properties.reportMissedCallbacks)
			{
				std::cout << "Warning <" << _common_set_properties.moduleReference << ">: shortcut callback <" << hookname << "> for property "
