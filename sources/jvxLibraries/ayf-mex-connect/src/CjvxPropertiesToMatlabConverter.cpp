	...
	const char* descr, jvxSize offset, jvxAccessProtocol* accProt)
{
	...
	if (formatFits)
	{
		...
		CjvxMatlabToCConverter::convert_mat_buf_c_buf_1_x_N(data_setprop, format, numElms, prhs);
	...
	case JVX_DATAFORMAT_SELECTION_LIST:
		...
		if (res == JVX_NO_ERROR)
		{
			...
			if (res == JVX_NO_ERROR)
			{
				...
				if (res == JVX_NO_ERROR)
				{
					// Here, we have found all arguments to run a (contentOnly == false) run
					selList.strList.assign(tokenLst);
