...
public:
	...
	IjvxObject* theTextLogger_obj;
	IjvxTextLog* theTextLogger_hdl;
	jvxSize dbgLevel;
	std::string theModuleName;
	jvxBool dbgModule;
	jvxostream jvxos;
	char* jvxlst_buf;
	jvxSize jvxlst_buf_sz;
...
class jvx_test_settings
{
public:
	jvxSize buffersize;
	jvxSize samplerate;
	jvxSize numberinputchannels;
	jvxSize numberoutputchannels;
	jvxDataFormat format;
	jvxSize segmentsize_x;
	jvxSize segmentsize_y;
...
public:
	jvx_test_settings()
	{
		...
		numberinputchannels = JVX_SIZE_UNSELECTED;
		numberoutputchannels = JVX_SIZE_UNSELECTED;
		format = JVX_DATAFORMAT_NONE;
		segmentsize_x = JVX_SIZE_UNSELECTED;
		segmentsize_y = JVX_SIZE_UNSELECTED;
		subformat = JVX_DATAFORMAT_GROUP_NONE;
	};

// #if 0
	static jvxErrorType test_connect_icon_flex(jvxLinkDataDescriptor* theData_in, jvxCommonSetNodeParams* node_params,
		IjvxOutputConnector* refto, jvx_test_settings* set  JVX_CONNECTION_FEEDBACK_TYPE_A(fdb))
	{
		jvxSize i;
		jvxLinkDataDescriptor ld_cp;
		JVX_CONNECTION_FEEDBACK_ON_ENTER_CONN(fdb, refto);

		jvxBool thereismismatch = false;
		jvxSize num = node_params->allowedSetup.size();
		jvxErrorType resComplain = JVX_NO_ERROR;
		std::string txt;

		//jvxBool anyModification = false;
		...
		if (num)
		{
			...
			{
				if (
					(node_params->allowedSetup[i].buffersize == theData_in->con_params.buffersize) &&
			...
			if (thereismismatch)
			{
				ld_cp.con_params.buffersize = node_params->allowedSetup[i].buffersize;
		...
		else
		{
			...
			{
				if (theData_in->con_params.buffersize < node_params->preferred.buffersize.min)
				{
					thereismismatch = true;
					ld_cp.con_params.buffersize = node_params->preferred.buffersize.min;
				}
			}
			if (JVX_CHECK_SIZE_SELECTED(node_params->preferred.buffersize.max))
			{
				if (theData_in->con_params.buffersize > node_params->preferred.buffersize.max)
				{
					thereismismatch = true;
					ld_cp.con_params.buffersize = node_params->preferred.buffersize.max;
				}
			}

			if (JVX_CHECK_SIZE_SELECTED(node_params->preferred.samplerate.min))
			{
				if (theData_in->con_params.rate < node_params->preferred.samplerate.min)
				{
					thereismismatch = true;
					ld_cp.con_params.rate = node_params->preferred.samplerate.min;
				}
			}
			if (JVX_CHECK_SIZE_SELECTED(node_params->preferred.samplerate.max))
			{
				if (theData_in->con_params.rate > node_params->preferred.samplerate.max)
				{
					thereismismatch = true;
					ld_cp.con_params.rate = node_params->preferred.samplerate.max;
				}
			}

			if (JVX_CHECK_SIZE_SELECTED(node_params->preferred.number_input_channels.min))
			{
				if (theData_in->con_params.number_channels < node_params->preferred.number_input_channels.min)
				{
					thereismismatch = true;
					ld_cp.con_params.number_channels = node_params->preferred.number_input_channels.min;
				}
			}
			if (JVX_CHECK_SIZE_SELECTED(node_params->preferred.number_input_channels.max))
			{
				if (theData_in->con_params.number_channels > node_params->preferred.number_input_channels.max)
				{
					thereismismatch = true;
					ld_cp.con_params.number_channels = node_params->preferred.number_input_channels.max;
				}
			}

			if (node_params->preferred.format.min != JVX_DATAFORMAT_NONE)
			{
				if (theData_in->con_params.format < node_params->preferred.format.min)
				{
					thereismismatch = true;
					ld_cp.con_params.format = node_params->preferred.format.min;
				}
			}
			if (node_params->preferred.format.max != JVX_DATAFORMAT_NONE)
			{
				if (theData_in->con_params.format > node_params->preferred.format.max)
				{
					thereismismatch = true;
					ld_cp.con_params.format = node_params->preferred.format.max;
				}
			}
		...
		case JVX_NO_ERROR:
			...
			set->buffersize = (theData_in->con_params.buffersize);
		...
		default:
			...
			txt += JVX_CONNECTION_FEEDBACK_GET_ERROR_STRING(fdb);
	...
	static void init_connect_icon_flex(jvxCommonSetNodeParams* node_params)
	{

		// ===================================================================================
		node_params->acceptOnlyExactMatchLinkDataInput = false;
		node_params->acceptOnlyExactMatchLinkDataOutput = false;

		node_params->preferred.buffersize.min = JVX_SIZE_UNSELECTED;
		node_params->preferred.buffersize.max = JVX_SIZE_UNSELECTED;
		node_params->preferred.number_input_channels.min = JVX_SIZE_UNSELECTED;
		node_params->preferred.number_input_channels.max = JVX_SIZE_UNSELECTED;
		node_params->preferred.number_output_channels.min = JVX_SIZE_UNSELECTED;
		node_params->preferred.number_output_channels.max = JVX_SIZE_UNSELECTED;
		node_params->preferred.samplerate.min = JVX_SIZE_UNSELECTED;
		node_params->preferred.samplerate.max = JVX_SIZE_UNSELECTED;
		node_params->preferred.format.min = JVX_DATAFORMAT_NONE;
		node_params->preferred.format.max = JVX_DATAFORMAT_NONE;

		node_params->preferred.subformat.min = JVX_DATAFORMAT_GROUP_NONE;
		node_params->preferred.subformat.max = JVX_DATAFORMAT_GROUP_NONE;
		node_params->preferred.dimX.min = JVX_SIZE_UNSELECTED;
		node_params->preferred.dimX.max = JVX_SIZE_UNSELECTED;
		node_params->preferred.dimY.min = JVX_SIZE_UNSELECTED;
		node_params->preferred.dimY.max = JVX_SIZE_UNSELECTED;

		node_params->processing.buffersize = JVX_SIZE_UNSELECTED;
		node_params->processing.number_input_channels = JVX_SIZE_UNSELECTED;
		node_params->processing.number_output_channels = JVX_SIZE_UNSELECTED;
		node_params->processing.samplerate = JVX_SIZE_UNSELECTED;
		node_params->processing.format = JVX_DATAFORMAT_NONE;

		node_params->settingReadyToStart = true;
	};

	 jvxBool anythingChanged(const jvx_test_settings& compareme)
	{
		if (compareme.buffersize != buffersize)
			return true;
		if (compareme.samplerate != samplerate)
			return true;
		if (compareme.format != format)
			return true;
		if (compareme.numberinputchannels != numberinputchannels)
			return true;
		if (compareme.numberoutputchannels != numberoutputchannels)
			return true;
		if (compareme.segmentsize_x != segmentsize_x)
			return true;
		if (compareme.segmentsize_y != segmentsize_y)
			return true;
		if (compareme.subformat != subformat)
			return true;
