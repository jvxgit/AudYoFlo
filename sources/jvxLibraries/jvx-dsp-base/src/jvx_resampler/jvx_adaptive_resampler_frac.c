...
typedef enum
  ...
  } jvxAdaptiveResampler_frac_config_stopband;
...
jvxDspBaseErrorType
jvxAdaptiveResampler_frac_fir_fragment(jvxInt32 midPosition, ars_frac_vII* handle)
{
  int i,j;
 ...
 *///=========================================================================================================
jvxDspBaseErrorType
jvxAdaptiveResampler_frac_variantI_initialize(jvxHandle** H, jvxData* delay_samples, int lagrange_toLeft, int lagrange_toRight, int lFilter, int buffersizeInMax, int oversamplingFactor, jvxDataFormat format)
{
  int i,j;
  ars_frac_vI* handle;
  ...
  JVX_DSP_SAFE_ALLOCATE_OBJECT_Z(handle, ars_frac_vI);
  ...
  handle->ram.fracSamplesFromLast = 0.0;
...
jvxDspBaseErrorType
jvxAdaptiveResampler_frac_variantI_terminate(jvxHandle* H)
{
  int i;
  // ====================================================================
  // Start config
  ...
  ars_frac_vI* handle = (ars_frac_vI*)H;
...
jvxDspBaseErrorType
jvxAdaptiveResampler_frac_variantI_process(void* in, int bSizeIn, void* out, int* bSizeOut, jvxData desiredBSizeOut_frac, jvxHandle* H)
{
  int i,j,k;
  ...
  outCnt = 0;
  idxVal = handle->ram.fracSamplesFromLast;
  shiftFac = (buffersizeUpsampled_local/desiredBSizeOut_frac);
  idxOffset = (jvxInt32)floor(idxVal);

  switch(handle->constants.format)
    {
    case JVX_DATAFORMAT_DATA:

      // If there is one sample from last frame, the first part is zero. Otherwise it is the oversampling factor
      // The fractional part will be taken from last frame and should be smaller than oversampling

      while(idxOffset < buffersizeUpsampled_local)
        {
          jvxData idxDelta = idxVal - idxOffset;
      ...
      handle->ram.fracSamplesFromLast = idxVal - buffersizeUpsampled_local;
      *bSizeOut = outCnt;
      break;
...
jvxDspBaseErrorType
jvxAdaptiveResampler_frac_variantII_initialize(jvxHandle** H, jvxData* delay_samples, int lagrange_toLeft, int lagrange_toRight, int lFilter, int buffersizeInMax, int oversamplingFactor, jvxDataFormat format)
{
  int i,j;
  ars_frac_vII* handle;
  ...
  JVX_DSP_SAFE_ALLOCATE_OBJECT_Z(handle, ars_frac_vII);
...
jvxDspBaseErrorType
jvxAdaptiveResampler_frac_variantII_terminate(jvxHandle* H)
{
  int i;
  // ====================================================================
  // Start config
  ...
  ars_frac_vII* handle = (ars_frac_vII*)H;
...
jvxDspBaseErrorType
jvxAdaptiveResampler_frac_variantII_process(void* in, int bSizeIn, void* out, int* bSizeOut, jvxData desiredBSizeOut_frac, jvxHandle* H)
{
  int i,j,k;
      ...
      while(idxOffset < buffersizeUpsampled_local)
        {
          ...
          jvxInt32 midPositionLagrange = idxOffset - handle->constants.lagrange_toRight;

          jvxData accuOut = 0;

          // Get the important fragment in upsampled data
          jvxAdaptiveResampler_frac_fir_fragment(midPositionLagrange, handle);
...
jvxDspBaseErrorType
jvxAdaptiveResampler_frac_variantIII_initialize(jvxHandle** H, jvxData* delay_samples, int lagrange_toLeft, int lagrange_toRight, int lFilter, int buffersizeInMax, int oversamplingFactor, jvxDataFormat format)
{
  int i,j;
  ars_frac_vIII* handle;
  int cntO;
  ...
  JVX_DSP_SAFE_ALLOCATE_OBJECT_Z(handle, ars_frac_vIII);
  ...
  // ====================================================================
  // For fractional number of output samples
  // ====================================================================
  handle->ram.fracSamplesFromLast = 0.0;
...
jvxDspBaseErrorType
jvxAdaptiveResampler_frac_variantIII_terminate(jvxHandle* H)
{
  int i;
  // ====================================================================
  // Start config
  ...
  ars_frac_vIII* handle = (ars_frac_vIII*)H;
...
jvxDspBaseErrorType
jvxAdaptiveResampler_frac_variantIII_process(void* in, int bSizeIn, void* out, int* bSizeOut, jvxData desiredBSizeOut_frac, jvxHandle* H)
{
  int i,j,k;
          ...
          for(i = 0; i < buffersize_local; i++)
            {
              ...
              *ptrTmpOut++  = JVX_DATA_2_DATA(inDbl[i]);
            }
          break;
        case JVX_DATAFORMAT_32BIT_LE:
          // Loop over input samples
