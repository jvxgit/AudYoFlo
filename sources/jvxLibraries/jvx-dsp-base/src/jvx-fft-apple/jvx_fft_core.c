						    ...
						    jvxSize fftSizeArbitrary)
{
	...
	if(hdl && glob_ptr)
	{
		...
		if((fftType == JVX_FFT_TOOLS_FFT_ARBITRARY_SIZE) || (fftSizeArbitrary != 0))
		{
	...
	if(hdl)
	{
		...
		if(ptr)
		{
			...
			if(res == JVX_DSP_NO_ERROR)
			{
			  ...
			  /*
			    if(!global_hdl->setupFFT.isPureRadix2)
				{
					assert(0);
				}
			  */
...
#ifdef JVX_FFT_APPLE
			  /*
				if(!ptr->fftParameters.isPureRadix2)
				{
					assert(0);
				}
			  */
...
jvx_fft_tools_get_nearest_size(jvxSize n)
{
	...
	return (jvxSize)ceil(log((jvxData)n)/log(2.)) - 4;
}
