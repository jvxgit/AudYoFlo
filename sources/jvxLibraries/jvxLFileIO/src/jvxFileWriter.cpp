...
jvxErrorType
jvxFileWriter::activate(std::string fName, jvxEndpointClass descrEndpoint, jvxFileDescriptionEndpoint_open* fileDescr)
{
		...
		do
		{
			if(resW == JVX_WAIT_SUCCESS)
			{
				while (1)
				{
					...
					res = read_from_circ_buffer(samplesRead, buffer_realtime.idxRead);
