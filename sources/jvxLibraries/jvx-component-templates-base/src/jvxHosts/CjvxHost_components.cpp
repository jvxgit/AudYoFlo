...
jvxErrorType 
CjvxHost::static_local_select(IjvxHiddenInterface* theHinterface,
...
jvxErrorType 
CjvxHost::static_local_unselect(IjvxHiddenInterface* theHinterface,
					  IjvxCoreStateMachine* theObjectSm)
{
	jvxErrorType res = JVX_NO_ERROR;
	assert(theObjectSm);
...
jvxErrorType
CjvxHost::number_components_system(const jvxComponentIdentification& tp, jvxSize* numPtr)
{
 ...
 */
jvxErrorType
CjvxHost::name_component_system(const jvxComponentIdentification& tp, jvxSize idx, jvxApiString* str)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
 ...
 */
jvxErrorType
CjvxHost::description_component_system(const jvxComponentIdentification& tp, jvxSize idx, jvxApiString* str)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
 ...
 */
jvxErrorType
CjvxHost::descriptor_component_system(const jvxComponentIdentification& tp, jvxSize idx, jvxApiString* str, jvxApiString* substr)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
	std::vector<oneTechnologyType>::iterator elmIt_tech;
	elmIt_tech = jvx_findItemSelectorInList_one<oneTechnologyType, jvxComponentType>(_common_set_types.registeredTechnologyTypes, tp.tp, 0);
 ...
 */
jvxErrorType
CjvxHost::module_reference_component_system(const jvxComponentIdentification& tp, jvxSize idx, jvxApiString* str, jvxComponentAccessType* acTp)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
 ...
 */
jvxErrorType
CjvxHost::feature_class_component_system(const jvxComponentIdentification& tp, jvxSize idx, jvxBitField* ft)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
	jvxSize num = 0;
	...
	jvxSize* szSubSlots_current, jvxSize* szSlots_max, jvxSize* szSubSlots_max)
{
	...
	if (res == JVX_ERROR_ELEMENT_NOT_FOUND)
	{
		...
		{
			res = JVX_NO_ERROR;
			this->number_connections_process(&num);
			int slotidmax = -1;
			for (jvxSize i = 0; i < num; i++)
			{
				...
				this->reference_connection_process(i, &conProc);
				...
				this->return_reference_connection_process(conProc);
				conProc = NULL;
			}
			if (szSlots_current)
			{
		...
		{
			res = JVX_NO_ERROR;
			this->number_connections_group(&num);
			int slotidmax = -1;
			for (jvxSize i = 0; i < num; i++)
			{
				...
				IjvxObject* objRef = NULL;
				jvxComponentIdentification tp;
				this->reference_connection_group(i, &conGrp);
				...
				this->return_reference_connection_group(conGrp);
				conGrp = NULL;
			}
			if (szSlots_current)
			{
...
jvxErrorType 
CjvxHost::set_number_subslots_system(const jvxComponentIdentification& tp, jvxSize newVal)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
	std::vector<oneTechnologyType>::iterator elmIt_tech;
 ...
 */
jvxErrorType
CjvxHost::module_reference_selected_component(const jvxComponentIdentification& tp, jvxApiString* str, jvxComponentAccessType* acTp)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
 ...
 */
jvxErrorType
CjvxHost::description_selected_component(const jvxComponentIdentification& tp, jvxApiString* str)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
 ...
 */
jvxErrorType
CjvxHost::descriptor_selected_component(const jvxComponentIdentification& tp, jvxApiString* str, jvxApiString* substr)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
	...
	if (res == JVX_ERROR_ELEMENT_NOT_FOUND) 
	{
			...
			(tp.tp == JVX_COMPONENT_HOST) && (tp.slotid == 0) && (tp.slotsubid == 0))
		{
			...
			if (str)
			{
				str->assign(_common_set.theDescriptor);
			}
		}
	}
 ...
 */
jvxErrorType
CjvxHost::feature_class_selected_component(const jvxComponentIdentification& tp, jvxBitField* ft)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
	jvxSize num = 0;
	jvxState stat = JVX_STATE_NONE;
 ...
 */
jvxErrorType
CjvxHost::select_component(jvxComponentIdentification& tp, jvxSize idx,
 ...
 */
jvxErrorType
CjvxHost::selection_component(const jvxComponentIdentification& tp, jvxSize* idRet)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
 ...
 */
jvxErrorType
CjvxHost::activate_selected_component(const jvxComponentIdentification& tp)
{
				...
				else
				{
					if (elmIt_tech->technologyInstances.selTech[tp.slotid].theHandle_shortcut_tech)
					{
						...
						if (res != JVX_NO_ERROR)
						{
							...
							this->_common_set.theErrordescr = (std::string)"Failed to activate technology type <" + elmIt_tech->description + ">, technology <" +
					...
					if (tp.slotsubid < elmIt_dev->technologyInstances.selTech[tp.slotid].theHandle_shortcut_dev.size())
					{
						if (elmIt_dev->technologyInstances.selTech[tp.slotid].theHandle_shortcut_dev[tp.slotsubid].dev)
						{
							...
							if (res != JVX_NO_ERROR)
							{
								...
								this->_common_set.theErrordescr = (std::string)"Failed to activate device type <" + elmIt_dev->description + ">, device <" +
									theName.std_str() + ">, reason: " + theErr.errorDescription.std_str();
							}
							else
							{
								...
								if (res != JVX_NO_ERROR)
								{
									this->_report_text_message("Failed to add connector and/or connector master factory for device", JVX_REPORT_PRIORITY_WARNING);
								}
							}
							/*
							else
...
jvxErrorType
CjvxHost::is_ready_selected_component(const jvxComponentIdentification& tp, jvxBool* ready, jvxApiString* reasonIfNot)
{
	std::string reason;
 ...
 */
jvxErrorType
CjvxHost::state_selected_component(const jvxComponentIdentification& tp, jvxState* st)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
	jvxState stat = JVX_STATE_NONE;
 ...
 */
jvxErrorType
CjvxHost::deactivate_selected_component(const jvxComponentIdentification& tp)
{
	jvxSize k;
	jvxState stat = JVX_STATE_NONE;
 ...
 */
jvxErrorType
CjvxHost::unselect_selected_component(jvxComponentIdentification& tp)
{
	int h; // must be int!
	jvxState stat = JVX_STATE_NONE;
	...
	{		
		...
		state_selected_component(tp, &stat);
		if (stat >= JVX_STATE_ACTIVE)
		{
			res = deactivate_selected_component(tp);
		}
				...
				else
				{
					if (elmIt_tech->technologyInstances.selTech[tp.slotid].theHandle_shortcut_tech != NULL)
					{
						...
						if (res == JVX_NO_ERROR)
						{
							...
							_release_unique_id(elmIt_tech->technologyInstances.selTech[tp.slotid].uid);
							tp.uId = JVX_SIZE_UNSELECTED;
				...
				if (tp.slotid < elmIt_dev->technologyInstances.selTech.size())
				{
					if (tp.slotsubid < elmIt_dev->technologyInstances.selTech[tp.slotid].theHandle_shortcut_dev.size())
					{
						if (elmIt_dev->technologyInstances.selTech[tp.slotid].theHandle_shortcut_dev[tp.slotsubid].dev != NULL)
						{
							...
							{
								// Release the unique id
								_release_unique_id(elmIt_dev->technologyInstances.selTech[tp.slotid].theHandle_shortcut_dev[tp.slotsubid].uid);
								tp.uId = JVX_SIZE_UNSELECTED;
...
jvxErrorType
CjvxHost::unique_id_selected_component(const jvxComponentIdentification& tp, jvxSize* uId)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
	...
	if(res == JVX_ERROR_ELEMENT_NOT_FOUND)
	{
			...
			(tp.tp == JVX_COMPONENT_HOST) && (tp.slotid == 0) && (tp.slotsubid == 0))
		{
			...
			if (uId)
			{
				*uId = this->_common_set.register_host_id;			
			}
		}
	}
...
jvxErrorType
CjvxHost::switch_state_component(const jvxComponentIdentification& cpId, jvxStateSwitch sswitch)
{
	jvxErrorType res = JVX_ERROR_ELEMENT_NOT_FOUND;
