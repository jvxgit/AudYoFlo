...
CjvxAudioDevice::CjvxAudioDevice(JVX_CONSTRUCTOR_ARGUMENTS_MACRO_DECLARE):
	CjvxSimpleMasterDevice(JVX_CONSTRUCTOR_ARGUMENTS_MACRO_CALL), CjvxProperties(module_name, *this)
{
	...
	/*
	CjvxDeviceCaps_genpcg::init_all();
	CjvxDeviceCaps_genpcg::allocate_all();
	CjvxDeviceCaps_genpcg::register_all(static_cast<CjvxProperties*>(this));
	*/
	// Typically, the devices are duplex devices :-)
	//CjvxDeviceCaps_genpcg::translate__device_caps__dev_caps_to(
	//	jvxDeviceCapabilities::JVX_DEVICE_SINGLE_ACTIVE_DUPLEX);
...
CjvxAudioDevice::~CjvxAudioDevice()
{
	...
	/*
	CjvxDeviceCaps_genpcg::unregister_all(static_cast<CjvxProperties*>(this));
	CjvxDeviceCaps_genpcg::deallocate_all();
	*/
}

jvxErrorType
CjvxAudioDevice::select(IjvxObject* owner)
{
...
*/
		...
		//_update_property_access_type(JVX_PROPERTY_ACCESS_READ_ONLY, JVX_PROPERTY_CATEGORY_PREDEFINED, CjvxAudioDevice::properties_active.format.globalIdx);
	...
	const jvx::propertyDetail::CjvxTranferDetail& detail)
{
	...
	if(res == JVX_NO_ERROR)
	{
		...
		this->updateDependentVariables_lock(propId, category, false, callGate.call_purpose);
	}
	return(res);
}

jvxErrorType 
CjvxAudioDevice::stop_property_group(jvxCallManagerProperties& callGate)
{
...
jvxErrorType
CjvxAudioDevice::put_configuration(jvxCallManagerConfiguration* callConf,
	...
	const char* filename, jvxInt32 lineno)
{
	...
	if(this->_common_set_min.theState == JVX_STATE_ACTIVE)
	{
		...
		CjvxAudioDevice::put_configuration__properties_active(callConf, processor, sectionToContainAllSubsectionsForMe, &warnings);
		if(!warnings.empty())
		{
			jvxSize i;
			std::string txt;
...
jvxErrorType
CjvxAudioDevice::get_configuration(jvxCallManagerConfiguration* callConf, 
	IjvxConfigProcessor* processor, jvxHandle* sectionWhereToAddAllSubsections)
{
	...
	{
		CjvxAudioDevice::get_configuration__properties_active(callConf, processor, sectionWhereToAddAllSubsections);
	}
	return(JVX_NO_ERROR);
}

void
CjvxAudioDevice::updateDependentVariables_lock(jvxSize propId, 
	...
	jvxBool suppress_update_chain)
{
	...
	{
		if(
			(propId == CjvxAudioDevice::properties_active.inputchannelselection.globalIdx) ||
			(propId == CjvxAudioDevice::properties_active.numberinputchannels.globalIdx) ||
			updateAll)
		{
		...
		if(
			(propId == CjvxAudioDevice::properties_active.outputchannelselection.globalIdx) ||
			(propId == CjvxAudioDevice::properties_active.numberoutputchannels.globalIdx) ||
			updateAll)
		{
			_lock_properties_local();
			newValue = 0;
		...
		if(
			(propId == CjvxAudioDevice::properties_active.format.globalIdx) && (!updateAll))
		{
			_lock_properties_local();
		...
		if(
			(propId == CjvxAudioDevice::properties_active.formatselection.globalIdx) ||
			updateAll || formatFromSelectionList)
		{
			...
			for(i = 0; i < CjvxAudioDevice::properties_active.formatselection.value.entries.size(); i++)
			{
				...
				{
					if(jvx_bitTest(CjvxAudioDevice::properties_active.formatselection.value.selection(), i))
					{
		...
		if (
			(propId == CjvxAudioDevice::properties_active.activateSilenceStop.globalIdx) || updateAll)
		{
			updateChain = true;
		}

		if (
			(propId == CjvxAudioDevice::properties_active.periodSilenceStop.globalIdx) || updateAll)
		{
			updateChain = true;
		}
	}
...
jvxErrorType
CjvxAudioDevice::currentSetupAudioParams(jvxAudioParams& params)
{
	jvxSize i;

	// Determine all relevant processing parameters
	params.samplerate = CjvxAudioDevice::properties_active.samplerate.value;
	params.buffersize = CjvxAudioDevice::properties_active.buffersize.value;
	params.format = (jvxDataFormat)CjvxAudioDevice::properties_active.format.value;
	...
	params.maxChanCntInput = CjvxAudioDevice::properties_active.inputchannelselection.value.entries.size();
	params.maxChanCntOutput = CjvxAudioDevice::properties_active.outputchannelselection.value.entries.size();

	params.inChannelsSelectionMask = CjvxAudioDevice::properties_active.inputchannelselection.value.selection();
	params.outChannelsSelectionMask = CjvxAudioDevice::properties_active.outputchannelselection.value.selection();
...
jvxErrorType
CjvxAudioDevice::test_chain_master(JVX_CONNECTION_FEEDBACK_TYPE(fdb))
{
	jvxErrorType res = JVX_NO_ERROR;
...
jvxErrorType 
CjvxAudioDevice::test_connect_icon(JVX_CONNECTION_FEEDBACK_TYPE(fdb)) 
{
...
jvxErrorType
CjvxAudioDevice::transfer_backward_ocon_match_setting(jvxLinkDataTransferType tp, jvxHandle* data, jvxPropertyContainerSingle<jvxSelectionList_cpp>* rateselection,
...
/*
jvxErrorType
CjvxAudioDevice::adaptNumberInputChannels(jvxSize numChannels)
{
