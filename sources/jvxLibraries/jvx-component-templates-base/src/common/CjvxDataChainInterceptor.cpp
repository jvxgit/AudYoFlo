...
jvxErrorType
CjvxDataChainInterceptor::supports_connector_class_icon(
...
jvxErrorType
CjvxDataChainInterceptor::supports_connector_class_ocon(
	jvxDataFormatGroup format_group,
	jvxDataflow data_flow)
{
	return JVX_NO_ERROR;
...
jvxErrorType 
CjvxDataChainInterceptor::reference_next_handle(jvxSize idx, IjvxConnectionIterator** next, jvxApiString* nmOcon, jvxApiString* nmIcon)
{
	if(idx == 0)
	{
		if (next)
...
/*
jvxErrorType
CjvxDataChainInterceptor::reference_next_ocon_name(jvxSize idx)
{
	if (idx == 0)
	{
