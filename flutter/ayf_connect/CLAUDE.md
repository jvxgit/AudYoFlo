...
### Edit vs. read-only canvas

`CanvasPage(mode: DiagramEditorMode.edit | DiagramEditorMode.readOnly)` (`lib/editor/diagram_editor_mode.dart`) toggles between the normal editable canvas and a read-only display. `fl_nodes` has **no config flag** to disable node-drag/link-drag gestures — they're unconditionally wired into its default node widget. Read-only mode therefore swaps in `ReadOnlyNode` (`lib/widgets/read_only_node.dart`) via `FlNodeEditorWidget.nodeBuilder`, which **fully replaces** fl_nodes' default node widget (including all its gesture handling) rather than configuring it — `ReadOnlyNode` reimplements the visual layout from scratch (header via the shared `DiagramNodeHeader`, ports) with only a plain `onTap` for selection, no drag/connect handling at all. It must preserve `node.key`/`port.key` (`GlobalKey`s) exactly as the default widget does, since link-line rendering depends on them to find port positions.
