...
import 'package:connectorflo/widgets/read_only_node.dart';

// ReadOnlyNode enthält DiagramNodeHeader, das seinen Titel über
...
// nodeBuilder übergibt. Da diese Tests ReadOnlyNode bewusst isoliert (ohne
...
void main() {
  testWidgets('Tippen auf ReadOnlyNode wählt den Node aus', (tester) async {
    ...
    await tester.pumpWidget(
      _wrapNode(ReadOnlyNode(controller: controller, node: node)),
    );
  ...
  testWidgets('Ziehen auf ReadOnlyNode verschiebt den Node nicht',
      (tester) async {
    final controller = _controllerWithSourceSinkPrototypes();
    addTearDown(controller.dispose);
