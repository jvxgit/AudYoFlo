...
macro(jvx_genMatProperties targetname componenttype componentprefix localfilelist localoptions)
	...
	if(JVX_MATLAB_PROPERTY_GENERATOR AND NOT JVX_RELEASE_SDK) 
...
macro(jvx_install_project_folders collect_install_subproject)
	if(NOT JVX_RELEASE_SDK OR JVX_FORCE_INSTALL_PROJECT_SDK)
		...
		if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/install)
			...
			set(target_folder ${CMAKE_CURRENT_SOURCE_DIR}/install/${JVX_OS})
			if(EXISTS ${target_folder})
				message("-1.A.i->Checking folder ${target_folder}/${host_identification}")
				if(EXISTS ${target_folder}/${host_identification})
					set(target_folder ${target_folder}/${host_identification})
					message("-1.A.ii->Found system specific folder ${target_folder}")
				endif()
				
				message("Install from folder ${target_folder} -> ${INSTALL_PATH_RUNTIME}")
				install(DIRECTORY ${target_folder}/ DESTINATION ${INSTALL_PATH_RUNTIME} USE_SOURCE_PERMISSIONS)

				if(${collect_install_subproject})
					LIST(APPEND LOCAL_COLLECT_LIST "${target_folder}/")
				endif()
				
				if(JVX_COLLECT_LOCAL_INSTALL_BUNDLES)
					set(JVX_COLLECT_LOCAL_INSTALL_BUNDLES "${JVX_COLLECT_LOCAL_INSTALL_BUNDLES};${target_folder}/" CACHE INTERNAL "project install folders")
    			else()
					set(JVX_COLLECT_LOCAL_INSTALL_BUNDLES "${target_folder}/" CACHE INTERNAL "project install folders")
    			endif()
			endif()
			message("-1.B->Looking for folder ${CMAKE_CURRENT_SOURCE_DIR}/install/common")
			set(target_folder "${CMAKE_CURRENT_SOURCE_DIR}/install/common")
			if(EXISTS ${target_folder})
				message("-1.B.i->Checking folder ${target_folder}/${host_identification}")
				if(EXISTS ${target_folder}/${host_identification})
					set(target_folder ${target_folder}/${host_identification})
					message("-1.B.ii->Found system specific folder ${target_folder}")
				endif()
				message("Install from folder ${target_folder} -> ${INSTALL_PATH_RUNTIME}")
				install(DIRECTORY ${target_folder}/ DESTINATION ${INSTALL_PATH_RUNTIME} USE_SOURCE_PERMISSIONS)
