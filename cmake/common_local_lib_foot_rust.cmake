set(CARGO_TARGET_DIR "${CMAKE_CURRENT_BINARY_DIR}")

set(CARGO_BUILD_TARGET_FOLDER "")
if(CARGO_BUILD_TARGET)
	set(CARGO_BUILD_OPTIONS "--target \" ${CARGO_BUILD_TARGET}\"")
	set(CARGO_BUILD_TARGET_FOLDER "${CARGO_BUILD_TARGET}/")
endif()

if(CMAKE_BUILD_TYPE STREQUAL "Debug")
	if(${JVX_OS} MATCHES "windows")
		set(RUST_STATIC_LIB_PATH "${CARGO_TARGET_DIR}/${CARGO_BUILD_TARGET_FOLDER}debug/${PROJECT_NAME}.${JVX_STATIC_EXTENSION}")
	else()
		set(RUST_STATIC_LIB_PATH "${CARGO_TARGET_DIR}/${CARGO_BUILD_TARGET_FOLDER}debug/lib${PROJECT_NAME}.${JVX_STATIC_EXTENSION}")
	endif()
  set(CARGO_BUILD_OPTIONS "${CARGO_BUILD_OPTIONS}")
else()
	if(${JVX_OS} MATCHES "windows")
		set(RUST_STATIC_LIB_PATH "${CARGO_TARGET_DIR}/${CARGO_BUILD_TARGET_FOLDER}release/${PROJECT_NAME}.${JVX_STATIC_EXTENSION}")		
	else()
		set(RUST_STATIC_LIB_PATH "${CARGO_TARGET_DIR}/${CARGO_BUILD_TARGET_FOLDER}release/lib${PROJECT_NAME}.${JVX_STATIC_EXTENSION}")
	endif()
  set(CARGO_BUILD_OPTIONS "${CARGO_BUILD_OPTIONS} -r")
endif()

if(JVX_SYSTEM_USE_DATA_FORMAT_FLOAT)
  set(_CARGO_FEATURES "${LOCAL_CARGO_FEATURES} jvx-system-use-data-format-float")
else()
  set(_CARGO_FEATURES "${LOCAL_CARGO_FEATURES}")
endif()

if(NOT "${_CARGO_FEATURES}" STREQUAL "")
  set(CARGO_BUILD_OPTIONS "${CARGO_BUILD_OPTIONS} -F \"${_CARGO_FEATURES}\"")
endif()

if(DEFINED LOCAL_CARGO_OPTIONS AND NOT "${LOCAL_CARGO_OPTIONS}" STREQUAL "")
  set(CARGO_BUILD_OPTIONS "${CARGO_BUILD_OPTIONS} ${LOCAL_CARGO_OPTIONS}")
endif()

string(STRIP "${CARGO_BUILD_OPTIONS}" CARGO_BUILD_OPTIONS)

add_library(${JVX_TARGET_NAME_STATIC} STATIC IMPORTED GLOBAL)
set_target_properties(${JVX_TARGET_NAME_STATIC} PROPERTIES IMPORTED_LOCATION "${RUST_STATIC_LIB_PATH}")
target_link_libraries(${JVX_TARGET_NAME_STATIC} INTERFACE ${JVX_RUST_ADDITIONAL_LIBS})

add_custom_target(
  cargo_build__${PROJECT_NAME}
  BYPRODUCTS ${RUST_STATIC_LIB_PATH}
  COMMAND cargo build --target-dir ${CARGO_TARGET_DIR} --lib ${CARGO_BUILD_OPTIONS}
  WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
  COMMENT "Build STATIC Rust Library ${JVX_TARGET_NAME_STATIC} with options: ${CARGO_BUILD_OPTIONS}"
)

# message(FATAL_ERROR "Build Options: ${CARGO_BUILD_OPTIONS}")

# Generate the C header bindings
add_custom_target(
  cargo_cbindgen__${PROJECT_NAME}
  BYPRODUCTS ${CMAKE_CURRENT_BINARY_DIR}/${PROJECT_NAME}.h
  COMMAND cbindgen --crate ${PROJECT_NAME} --lang c --quiet --output ${CMAKE_CURRENT_BINARY_DIR}/${PROJECT_NAME}.h
  WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
  COMMENT "Generating header file for Rust Library ${JVX_TARGET_NAME_STATIC}"
)

file(GLOB_RECURSE RUST_SOURCES
  "${CMAKE_CURRENT_SOURCE_DIR}/src/*.rs"
)

source_group("Rust sources" FILES ${RUST_SOURCES} ${CMAKE_CURRENT_BINARY_DIR}/${PROJECT_NAME}.h)

set_source_files_properties(${RUST_SOURCES} PROPERTIES HEADER_FILE_ONLY ON)
if(EXISTS ${CMAKE_CURRENT_BINARY_DIR}/${PROJECT_NAME}.h)
	set(RUST_SOURCES ${RUST_SOURCES} ${CMAKE_CURRENT_BINARY_DIR}/${PROJECT_NAME}.h)
endif()

target_sources(${JVX_TARGET_NAME_STATIC} INTERFACE ${RUST_SOURCES})

add_dependencies(${JVX_TARGET_NAME_STATIC} cargo_build__${PROJECT_NAME})
add_dependencies(${JVX_TARGET_NAME_STATIC} cargo_cbindgen__${PROJECT_NAME})

foreach(ONE_ADDITIONAL_TARGET ${JVX_CBINDGEN_ADDITIONAL_TARGETS})
	message("Rust adding additional cbindgen target ${ONE_ADDITIONAL_TARGET}")
	add_dependencies(${JVX_TARGET_NAME_STATIC} ${ONE_ADDITIONAL_TARGET})
endforeach()
	

